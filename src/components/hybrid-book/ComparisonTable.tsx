    ...
    <section className="py-20 bg-slate-50">
        <div className="container mx-auto px-6">
            <div className="text-center mb-12">
                <h2 className="text-3xl font-black text-slate-900">Tại sao gọi là "Sách Hybrid"?</h2>
                <p className="text-slate-500 mt-2">Sự nâng cấp toàn diện so với sách truyền thống và sách bút chấm đọc.</p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto items-center">
                {COMPARISON_DATA.map((item) => (
                    <div 
                        key={item.id}
                        className={`
                            relative rounded-2xl transition duration-300
                            ${item.isHighlight 
                                ? 'bg-blue-600 text-white p-8 shadow-xl md:-translate-y-4 border-4 border-blue-400/30 overflow-hidden z-10' 
                                : 'bg-white text-slate-800 p-6 border border-slate-100 opacity-80 hover:opacity-100 hover:shadow-lg'
                            }
                        `}
                    >
                        {item.isHighlight && (
